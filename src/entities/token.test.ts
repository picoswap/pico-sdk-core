...
describe('Token', () => {
  ...
  describe('#equals', () => {
    it('fails if address differs', () => {
      expect(new Token(ChainId.EDGEWARE, ADDRESS_ONE, 18).equals(new Token(ChainId.EDGEWARE, ADDRESS_TWO, 18))).toBe(
        false
      )
    })

    it('false if chain id differs', () => {
      expect(new Token(ChainId.BERESHEET, ADDRESS_ONE, 18).equals(new Token(ChainId.EDGEWARE, ADDRESS_ONE, 18))).toBe(
        false
      )
    })
    ...
    it('true even if name/symbol/decimals differ', () => {
      const tokenA = new Token(ChainId.EDGEWARE, ADDRESS_ONE, 9, 'abc', 'def')
      const tokenB = new Token(ChainId.EDGEWARE, ADDRESS_ONE, 18, 'ghi', 'jkl')
